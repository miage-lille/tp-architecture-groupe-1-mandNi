...
import { InMemoryUserRepository } from "../../core/adapters/user-repository.in-memory";
...
describe('Feature: Book a seat', () => {
    ...
    describe('Scenario: Webinar not found', () => {
        it('should throw a WebinarNotFound error', async () => {
            await expect(useCase.execute({ webinarId: 'non-existent-webinar', user }))
    ...
    describe('Scenario: Not enough seats', () => {
        it('should throw a WebinarNotEnoughSeatsException', async () => {
            participationRepository.database.push(
                ...
                new Participation({ userId: 'user-charlie-id', webinarId: 'webinar-1' })
            );

            await expect(useCase.execute({ webinarId: 'webinar-1', user }))
